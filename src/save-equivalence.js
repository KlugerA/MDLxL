...
export function modelDifferences(expected,actual,{keys=Object.keys(expected),limit=12}={}) {
  const differences=[];
  function walk(a,b,path,key,integer=false) {
    if(differences.length>=limit || ignored.has(key))return;
    ...
    if(a==null||b==null) {if(a==null&&b==null || absentEquivalent(a??b,key))return;differences.push(path);return;}
    if(typeof a==='number'&&typeof b==='number'){if(integer||integerFields.has(key)?a!==b:!equalNumber(a,b))differences.push(path);return;}
    if(typeof a!=='object'||typeof b!=='object'){if(a!==b)differences.push(path);return;}
    if(ArrayBuffer.isView(a)||Array.isArray(a)) {
      if(a.length!==b.length){differences.push(path+'.length');return;}
...
export function assertModelEquivalent(expected,actual,options) {
  const differences=modelDifferences(expected,actual,options);
  if(differences.length)throw new Error(`Save verification failed: serialization changed ${differences.join(', ')}.`);
}
