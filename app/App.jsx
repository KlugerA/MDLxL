...
export default function App() {
  ...
  const [attachSourceId, setAttachSourceId] = useState(null), [boneCreateOpen, setBoneCreateOpen] = useState(false);
  ...
  const finishAttach = parentId => {
    if (attachSourceId == null) return;
    const result = edit('Attach to parent bone', ['Nodes'], current => attachToBone(current, attachSourceId, parentId));
    if (result !== false) setAttachSourceId(null);
  };
  const detachSelectedNode = () => {
    if (!singleRigNode || !parentBone) return;
    edit('Detach from parent bone', ['Nodes'], current => detachFromBone(current, singleRigNode.ObjectId));
  };
  ...
  const selectMode = async next => {
    ...
    if (next !== 'bones') { setAttachSourceId(null); setBoneCreateOpen(false); }
    if (next === 'bones') {
  ...
  const setWorkTool = next => {
    ...
    if (mode === 'bones') setAttachSourceId(null);
