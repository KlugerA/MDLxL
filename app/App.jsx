...
export default function App() {
  ...
  const setView = value => { setStoredView(value); if (quadView && mode === 'vertices') setVertexViewRequest(previous => ({ view: value, revision: (previous?.revision || 0) + 1 })); };
  const singleVertexView = useRef(view);
  const toggleQuadView = () => { if (!quadView) singleVertexView.current = view; else setView(singleVertexView.current); setQuadView(value => !value); };
  const freeVertexPlane = mode === 'vertices' && quadView && activeVertexPane && activeVertexPane.view !== 'perspective' && !activeVertexPane.workplane;
  ...
  return <WarmKeysProvider preferences={preferences} catalog={commandCatalog} activeScope={settingsTab?'settings':dialog?'dialog':mode==='paint'?'paint':'editor'} onAction={id=>runLatest.current(id)}><div data-vis-ui={visUI || undefined} data-warmkey-scope={mode==='paint'?'paint':'editor'} className={`classic-app${rigWorkspace?' rig-workspace':''}${activePortrait?' portrait-workspace':''}${mode==='uv'?' uv-mode':''}${mode==='paint'?' paint-mode':''}`} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const records = [...event.dataTransfer.files], record = records.find(file => /\.(mdl|mdx|mdlxlpaint)$/i.test(file.name)); if (record) withUnsaved(async () => { await loadFile(record); }); else loadTextures(records).catch(error => say(error.message, true)); }} onClick={() => context && setContext(null)}>
    ...
    <main className={`classic-workspace${mode === 'vertices' && quadView ? ' quad-workspace' : ''}${mode === 'animation' ? ' animation-workspace' : ''}${mode === 'uv' ? ' uv-immersive' : ''}${mode === 'paint' ? ' paint-immersive' : ''}`} inert={saving || undefined}><section className="classic-view">
      ...
      <fieldset className="classic-planes"><legend><label><input data-warmkey="workplaneEnabled" aria-label="Workplane" type="checkbox" disabled={!!lockedVertexPlane || !!freeVertexPlane} checked={!freeVertexPlane && (!!lockedVertexPlane || workplaneEnabled)} onChange={event => setWorkplaneEnabled(event.target.checked)}/>Workplane</label></legend>{[['xy', 'XY'], ['xz', 'ZX'], ['yz', 'YZ']].map(([value, label]) => <label key={value}><input data-warmkey={`plane:${value}`} aria-label={`${label} workplane`} type="radio" name="workplane" disabled={!!lockedVertexPlane || !!freeVertexPlane} checked={!freeVertexPlane && (lockedVertexPlane || workplane) === value} onChange={() => setWorkplane(value)}/>{label}</label>)}</fieldset>
