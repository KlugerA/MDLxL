...
export default function App() {
  ...
  const [mode, setMode] = useState('vertices'), [cameraMode, setCameraMode] = useState('work'), [view, setView] = useState('orthographic'), [workplane, setWorkplane] = useState('xy');
  ...
  const [hoveredGeoset, setHoveredGeoset] = useState(null);
  ...
  const enterPortrait = (cameraIndex, sequenceIndex) => {
    ...
    setCameraMode('work'); setPortraitEnabled(true); setPlaying(true);
  };
  ...
  const updatePortraitCamera = () => {
    ...
    if (index !== false) setPortraitCameraIndex(index);
    return index;
  };
  ...
  const openLibrary = (preview = mode === 'uv') => setDialog({ type: 'library', preview, returnTo: dialog?.type === 'resource' ? dialog : null });
  ...
  const moveNodes = payload => {
    ...
    return edit(restPose ? 'Move rest-pose nodes' : 'Transform bones and nodes', restPose ? ['Nodes','PivotPoints'] : ['Nodes'], m => applyMovementTransform(m, payload.nodeIds || selectedNodeIds, Math.round(payload.time ?? time), payload.sequenceIndex ?? sequence, change));
  };
  ...
  async function applyPaintToModel(){
    ...
    savingRef.current=true;setSaving(true);
    try{
      repairPaintMaterials(value,paintOriginalModel);
  ...
  function meshAction(name) {
    ...
    const updated = {}, removedGeosets = [], axis = workplane === 'xy' ? 2 : workplane === 'yz' ? 0 : 1;
  ...
  commands.current = { 'paint:select':()=>window.dispatchEvent(new CustomEvent('mdlxl-paint-tool',{detail:'select'})), 'paint:draw':()=>window.dispatchEvent(new CustomEvent('mdlxl-paint-tool',{detail:'draw'})), open, recent:showRecent, openRecent, clearRecent, save: () => mode==='paint'?savePaintProject():save(), saveAs: () => mode==='paint'?savePaintProject():setDialog({type:'saveFormat'}), new: () => withUnsaved(() => install(newSession(createStarterDocument(preferences.newModelVersion)))), recovery: listRecovery, gameData, undo: () => undo(false), redo: () => undo(true), copy, paste: () => paste(), pasteSpecial: () => { if (clipboard.current) { setAnchor(''); setDialog({ type: 'pasteSpecial' }); } }, selectAll, clear: () => { if(mode==='uv')return uvAction('select-none');setSelectedNodeIds([]); setSelection({}); }, history: showHistory, grid: () => setShowGrid(v => !v), frame: toggleTextured, frameSelection: () => setViewRenderMode('solid'), fit: () => frame(false), fitSelection: () => frame(true), vertices: () => selectMode('vertices'), bones: () => {setMovementMode('select');selectMode('bones');}, uv: () => selectMode('uv'), paint: () => {setRenderMode('textured');setView('perspective');selectMode('paint');}, animation: () => selectAnimationPanel('movement'), animations: () => selectAnimationPanel('animations'), textureLibrary: () => openLibrary(), help: () => setDialog({ type: 'help' }), diagnostics: () => setDialog({ type: 'diagnostics' }), about: () => setDialog({ type: 'about' }), ...Object.fromEntries(resources.map(kind => [kind, () => setDialog({ type: 'resource', kind })])), ...Object.fromEntries(views.map(name => [name, () => setView(name)])) };
  Object.assign(commands.current, {
    ...
    ...Object.fromEntries(['select','translate','rotate','scale'].map(v=>[v,()=>setWorkTool(v)])),
    ...Object.fromEntries(['xy','xz','yz'].map(v=>['plane:'+v,()=>setWorkplane(v)])),
    ...Object.fromEntries(['Delete vertices','Create triangle','Delete triangles','Uncouple','Collapse','Weld','Mirror','Extrude','Detach','Reverse normals','Smooth normals','Restore normals'].map(v=>[v,()=>mode==='animation' && v==='Collapse' ? timelineCommands.current.copyPose?.() : mode==='animation' && v==='Delete vertices' ? timelineCommands.current.delete?.() : meshAction(v)])),
  ...
  const commandCatalog = useMemo(() => COMMANDS.map(command => ({...command,allowInModal:settingsCommands.includes(command.id),enabled:commandEnabled(command.id)})), [doc, doc.revision, mode, tool, sequence, validSelection, selectable, hiddenCount, selectionCount, selectedFaces, geoset, clipboard.current, restrictions, restPose, rigWorkspace, cleanAnimationPreview, saving, repairBlocked, repairReceipt]);
  ...
  const geosetHighlightLabel=rigWorkspace?'Highlight Geoset':'Highlight Selection';
  ...
  return <WarmKeysProvider preferences={preferences} catalog={commandCatalog} activeScope={settingsTab?'settings':dialog?'dialog':mode==='paint'?'paint':'editor'} onAction={id=>runLatest.current(id)}><div data-vis-ui={visUI || undefined} data-warmkey-scope={mode==='paint'?'paint':'editor'} className={`classic-app${rigWorkspace?' rig-workspace':''}${activePortrait?' portrait-workspace':''}${mode==='uv'?' uv-mode':''}${mode==='paint'?' paint-mode':''}`} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const records = [...event.dataTransfer.files], record = records.find(file => /\.(mdl|mdx|mdlxlpaint)$/i.test(file.name)); if (record) withUnsaved(async () => { await loadFile(record); }); else loadTextures(records).catch(error => say(error.message, true)); }} onClick={() => context && setContext(null)}>
    ...
    <div className="classic-toolbar">
      ...
      {mode !== 'uv' && mode !== 'paint' && <QuickDisplay overlays={cleanAnimationPreview ? panelOverlays : overlays} shadows={shaded} particles={preferences.graphics.particles} cleanAnimationPreview={cleanAnimationPreview} onOverlay={changeOverlay} onShadows={setShaded} onParticles={showParticlePreview} onClear={clearDisplay}/>}
    ...
    <main className={`classic-workspace${mode === 'animation' ? ' animation-workspace' : ''}${mode === 'uv' ? ' uv-immersive' : ''}${mode === 'paint' ? ' paint-immersive' : ''}`} inert={saving || undefined}><section className="classic-view">
      ...
      <fieldset className="classic-planes"><legend><label><input data-warmkey="workplaneEnabled" aria-label="Workplane" type="checkbox" checked={workplaneEnabled} onChange={event => setWorkplaneEnabled(event.target.checked)}/>Workplane</label></legend>{[['xy', 'XY'], ['xz', 'ZX'], ['yz', 'YZ']].map(([value, label]) => <label key={value}><input data-warmkey={`plane:${value}`} aria-label={`${label} workplane`} type="radio" name="workplane" checked={workplane === value} onChange={() => setWorkplane(value)}/>{label}</label>)}</fieldset>
