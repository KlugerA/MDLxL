...
(async()=>{
  ...
  try{
    ...
    await waitFor(uv,`!!document.querySelector('.uv-preview-footer [aria-label="Selected Only"]')`);
    ...
    assert.deepEqual(buttons,['Select New','Live Select','Selected Only','Hide RGB']);
    ...
    await click(uv,'[aria-label="Selected Only"]');
