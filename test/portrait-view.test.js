...
test('evaluated camera application uses authored projection and fixed portrait aspect', () => {
  ...
  assert.ok(Math.abs(editorCameraSnapshot(camera, controls.target).fieldOfView-evaluated.fieldOfView)<1e-9);
});
...
test('camera edits subtract animated offsets instead of double-applying them', () => {
  ...
  updateModelCameraFromView(model, source, { position:[50,60,70], target:[8,9,10], fieldOfView:Math.PI/2, near:2, far:2000 }, 150, 0, 150);
  const evaluated = evaluateModelCamera(model, source, 150, 0, 150);
