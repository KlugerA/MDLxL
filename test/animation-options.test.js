...
test('shared geoset evaluator matches controls and native preview across sequences and scrubbing', () => {
  const model = {
    ...
    GeosetAnims: [{ GeosetId: 0, Flags: 2, Alpha: 0.5, Color: { LineType: 1, Keys: [key(100, rgbToWarcraftColor([1, 0, 0])), key(200, rgbToWarcraftColor([0, 0, 1])), key(500, rgbToWarcraftColor([0, 1, 0])), key(600, rgbToWarcraftColor([0, 1, 0]))] } }],
  ...
  for (const [frame, sequence, expected] of [[150, 0, [0.5, 0, 0.5]], [500, 1, [0, 1, 0]], [100, 0, [1, 0, 0]]]) {
    ...
    close(sample.color, rgbToWarcraftColor(expected)); assert.equal(sample.alpha, 0.5);
  ...
  model.GeosetAnims[0].Color = { LineType: 1, GlobalSeqId: 0, Keys: [key(0, rgbToWarcraftColor([1, 0, 0])), key(100, rgbToWarcraftColor([0, 0, 1]))] };
  close(sampleGeosetAnimation(model, 0, 550, 1, 125).color, rgbToWarcraftColor([0.75, 0, 0.25]));
  close(previewGeosetTint(model, 0, { Alpha: 1 }, 550, 1, 125), [0.75, 0, 0.25, 0.5]);
...
test('static RGB codec paths preserve channel order and evaluated appearance', () => {
  ...
  doc.apply('Set static RGB', ['GeosetAnims'], m => Object.assign(m.GeosetAnims[0], { Flags: 3, Alpha: 0.75, Color: rgbToWarcraftColor([0.8, 0.2, 0.4]) }));
  for (const format of ['mdl', 'mdx']) {
    ...
    close(evaluated.color, rgbToWarcraftColor([0.8, 0.2, 0.4]));
    close(sampleAnimationProperty(reopened.model, { kind: 'geoset', id: 0, property: 'Color' }, reopened.model.Sequences[0].Interval[0], 0), [0.8, 0.2, 0.4]); assert.equal(evaluated.alpha, 0.75);
  }
});
