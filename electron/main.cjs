...
async function createWindow(bounds={}){
  ...
  current.webContents.on('did-create-window',child=>{child.setMenu(null);child.webContents.setWindowOpenHandler(()=>({action:'deny'}));child.webContents.on('will-navigate',event=>event.preventDefault());});
